...
export default function RoomPage() {
  ...
  useEffect(() => {
    if (allVoted && !votesRevealed && voters.length > 0 && roomSettings.autoReveal) {
  ...
  const handleVote = async (value: string, confidence?: ConfidenceLevel) => {
    ...
    try {
      ...
      if (currentUser) {
        ...
        toast({
          title: "Vote cast",
          description: `You voted ${value}${confidence ? ` with ${confidence} confidence` : ""}`,
        })
      }
    } catch (error) {
  ...
  const results: VoteResult[] = participants
    ...
    .map((p) => ({ participant: p.name, vote: p.vote!, confidence: p.confidence }))

  const canRevealVotes = isModerator || roomSettings.allowOthersToShowEstimates
  const canStartNewRound = isModerator || roomSettings.allowOthersToDeleteEstimates
  ...
  useEffect(() => {
    ...
    autoJoin()
  }, [userName, participants.length, roomId, isMounted, currentUser, sendEvent])
  ...
  return (
    <div className="min-h-screen flex flex-col">
      ...
      <Header
        rightContent={
          userName ? (
            <UserMenu userName={userName} onNameChange={handleNameChange} onLeaveRoom={handleLeaveRoom} />
          ) : null
        }
      />

      <JoinDialog open={showJoinDialog} onJoin={handleJoin} isLoading={isJoining} />
      <RoomSettingsDialog
      ...
      <main id="room-content" role="main" aria-label="Planning poker room" className="flex-1 py-8">
        <div className="container mx-auto max-w-7xl px-4">
          <div className="mb-6">
            ...
            <div className="flex items-center justify-between flex-wrap gap-4">
              <div>
                <h1 className="text-2xl md:text-3xl font-bold" aria-label={`Room name: ${roomName}`}>
                  {isMounted ? roomName : "Loading..."}
                </h1>
                <p className="text-muted-foreground" aria-label={`Room ID: ${roomId}`}>
                  Room ID: {roomId}
                </p>
              </div>
              <div className="flex items-center gap-2" role="toolbar" aria-label="Room controls">
                <Button
                  variant="outline"
                  size="sm"
                ...
                <Button
                  ...
                  disabled={
                    votesRevealed
                      ...
                      : !allVoted || !canRevealVotes || isRevealing
                  }
                  aria-label={
                    votesRevealed
                      ...
                      : allVoted
                        ? "Reveal all votes"
                        : `Waiting for ${voters.length - votedCount} more vote${voters.length - votedCount !== 1 ? "s" : ""}`
                  }
                  aria-disabled={
                    votesRevealed
                      ? !canStartNewRound || isStartingNewRound
                ...
                >
                  {votesRevealed ? (
                    <>
                      <RotateCcw
                        className={`h-4 w-4 mr-2 ${isStartingNewRound ? "animate-spin" : ""}`}
                        aria-hidden="true"
                      />
          ...
          <div className="grid lg:grid-cols-3 gap-6">
            <div className="lg:col-span-2 space-y-6">
              <StoryInput currentStory={currentStory} onStoryUpdate={handleStoryUpdate} canEdit={true} />

              {!votesRevealed && voters.length > 0 && (
                <VotingProgress votedCount={votedCount} totalVoters={voters.length} />
              )}

              {timerDuration && roomSettings.showTimer && (
                <Card>
                  <CardContent className="pt-6">
                    <TimerDisplay duration={timerDuration} isActive={timerActive} onComplete={handleRevealVotes} />
                  </CardContent>
                </Card>
              )}
              ...
              {!currentUser?.isSpectator && (
                <Card>
                  <CardHeader>
                    ...
                    <CardDescription>
                      Select your estimate and confidence level for the current user story
                    </CardDescription>
                  </CardHeader>
                  <CardContent>
                    <div
                    ...
                    >
                      {deckValues.map((value) => (
                        <VotingCard
                          ...
                          onSelect={(confidence) => handleVote(value, confidence)}
                          disabled={votesRevealed || isVoting}
                        />
                      ))}
                    </div>
                    {selectedVote && !votesRevealed && (
                      <div className="text-center mt-4 space-y-1" role="status" aria-live="polite">
                        <p className="text-sm text-muted-foreground">
                          Your vote: <span className="font-bold text-primary">{selectedVote}</span>
                        </p>
                        {selectedConfidence && (
                          <p className="text-xs text-muted-foreground">
                            Confidence: <span className="font-medium capitalize">{selectedConfidence}</span>
                          </p>
                        )}
                      </div>
                    )}
            ...
            <div className="space-y-6">
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>
                    {voters.length} voter{voters.length !== 1 ? "s" : ""}, {participants.length - voters.length}{" "}
                    spectator
                    {participants.length - voters.length !== 1 ? "s" : ""}
                  </CardDescription>
                </CardHeader>
                <CardContent className="space-y-2" role="list" aria-label="Room participants">
                  {participants.map((participant) => (
                    <ParticipantCard key={participant.id} participant={participant} showVote={votesRevealed} />
                  ))}
              ...
              <Card>
                ...
                <CardContent className="space-y-3 text-sm" role="list" aria-label="Current room settings">
                  <div className="flex justify-between" role="listitem">
                    <span className="text-muted-foreground">Deck Type</span>
                    <span className="font-medium">{DECK_OPTIONS[deckType].label}</span>
                  </div>
                  {timerDuration && (
                    <div className="flex justify-between" role="listitem">
